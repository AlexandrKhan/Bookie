...
<div class="container">
    <ul class="responsive-table">
        <li class="table-header">
            <div class="col col-1"><fmt:message key="cabinet.id"/>
            </div>
            <div class="col col-2"><fmt:message key="cabinet.match"/>
            </div>
            <div class="col col-3"><fmt:message key="cabinet.score"/>
            </div>
            <div class="col col-4"><fmt:message key="cabinet.time"/>
            </div>
            <div class="col col-5"><fmt:message key="cabinet.result"/>
            </div>
            <div class="col col-6"><fmt:message key="cabinet.amount"/>
            </div>
            <div class="col col-7"><fmt:message key="cabinet.coefficient"/>
            </div>
        </li>
        <c:forEach items="${sessionScope.myBets}" var="bet">
            ...
            <c:forEach items="${sessionScope.myMatches}" var="match">
                <c:if test="${match.id == bet.matchId}">
                    <div class="col col-2" data-label="Match"><c:out
                            value="${match.homeTeam.name} - ${match.awayTeam.name}"/></div>
                    <div class="col col-3" data-label="Score"><c:out
                            value="${match.homeTeamGoals} - ${match.awayTeamGoals}"/></div>
                    <div class="col col-4" data-label="Match time"><c:out
                            value="${match.startTime}, ${match.startDate}"/></div>
                </c:if>
            </c:forEach>
